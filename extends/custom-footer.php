...
if(is_admin()){
	...
	add_action('wpjam_plugin_page_load', function($plugin_page, $current_tab){
		if($plugin_page == 'wpjam-posts' && $current_tab == 'posts'){
			add_filter('wpjam_option_setting_args', function($args){
				$args['sections']['posts']['fields']['custom-post']	= ['title'=>'文章页代码',	'type'=>'select',	'options'=>[0=>'不在文章列表页设置文章页代码', 1=>'在文章列表页设置文章页代码', 'only'=>'只在文章列表页设置文章页代码']];
